...
現状より約24%程の漁獲圧の減少によって、%SPR=30%を達成できることがわかった。これら一連の議論は、現状の漁業と同一の選択率を仮定している点に注意。F_multiplierは、同一の選択率の強弱を決めるスカラー値である。F_multiplier=1の時は、現状の漁獲係数を意味している（%SPR=21%）。
...
という順番に漁業状態および資源状態が変化することがわかる。管理指標値(Flimit, Blimit)や、様々なパラメータを変化させることで、
