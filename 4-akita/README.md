...
## 配布したRmdファイルはRstudioで開いてエディターの上部にある"knit"と書かれたボタンを押すと、数式やコードが綺麗に表示されるファイルを出力します。
## 配布したコードが文字化けする場合は、一旦メモ帳で開いてコピペすることで対応いただければと思います。
## アプリ内で指定することができない数字を変更したい場合は、app_future.Rのコードを変更することで対応できます。
